...
def generate_page(from_path, template_path, dest_path, project_root):
    ...
    completed_template = template.replace("{{ Content }}", content_html)
...
def generate_page_recursive(dir_path_content, template_path, dest_dir_path, project_root):
    ...
    for item in os.listdir(dir_path_content):
        ...
        if os.path.isfile(current_path):
            if current_path.endswith(".md"):
                generate_page(current_path, template_path, dest_dir_path, project_root)
        else:
            new_dest_dir = os.path.join(dest_dir_path, item)
            generate_page_recursive(current_path, template_path, new_dest_dir, project_root)